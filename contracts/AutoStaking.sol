...
contract AutoStaking is Ownable, Pausable {
    ...
    IMainStaking public immutable mainStaking;
     ...
     * @param _mainStaking: MasterChef contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_mainStaking), type(uint256).max);
    }
    ...
    function harvest() external notContract whenNotPaused {
        IMainStaking(mainStaking).leaveStaking(0);
    ...
    function emergencyWithdraw() external onlyAdmin {
        IMainStaking(mainStaking).emergencyWithdraw(0);
    }
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IMainStaking(mainStaking).pendingCake(0, address(this));
        amount += available();
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IMainStaking(mainStaking).leaveStaking(balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IMainStaking(mainStaking).userInfo(0, address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IMainStaking(mainStaking).enterStaking(bal);
        }
    }
