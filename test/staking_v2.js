...
contract (
    ...
    ([
        ...
        it('One pool math and workflow test', async () => {
            ...
            await STAKING.deposit(0, ONE_ETHER.mul(new BN(75)), user1, {from: user1});
            await STAKING.deposit(0, ONE_ETHER.mul(new BN(25)), user2, {from: user2});
            ...
            await expectRevert(STAKING.harvest(0, user1, {from: user1}), "Cannot harvest yet");

            await network.provider.send("evm_setNextBlockTimestamp", [(START_TIME.add(DAY.mul(new BN(3)).div(new BN(4)))).toNumber()]);

            await STAKING.deposit(0, ONE_ETHER.mul(new BN(50)), user2, {from: user2});

            REWARD = ONE_ETHER.mul(DAY).mul(new BN(4)).div(new BN(16));
            ...
            await STAKING.withdraw(0, 0, user1, {from: user1});
            ...
            await STAKING.withdraw(0, ONE_ETHER.mul(new BN(75)), user2, {from: user2});
            ...
            await expectRevert(STAKING.withdraw(0, ONE_ETHER.mul(new BN(75)), user1, {from: user1}), "Cannot withdraw yet");

            await network.provider.send("evm_setNextBlockTimestamp", [(START_TIME.add(DAY.mul(new BN(10)).div(new BN(4)))).toNumber()]);

            await STAKING.withdraw(0, ONE_ETHER.mul(new BN(75)), user1, {from: user1});
            ...
            await STAKING.deposit(0, ONE_ETHER.mul(new BN(50)), user3, {from: user3});

            await network.provider.send("evm_setNextBlockTimestamp", [(START_TIME.add(DAY.mul(new BN(5)))).toNumber()]);

            await STAKING.withdraw(0, ONE_ETHER.mul(new BN(50)), user3, {from: user3});
        ...
        it('Cross-pool math and workflow test', async () => {
            ...
            await STAKING.deposit(1, ONE_ETHER.mul(new BN(100)), user2, {from: user2});
            ...
            await STAKING.withdraw(1, ONE_ETHER.mul(new BN(100)), user2, {from: user2});
